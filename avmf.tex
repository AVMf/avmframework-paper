...
%
as well as performing test case selection~\cite{Pradhan2016} and prioritization~\cite{Arrieta2016}. % A. ARRIETA, S. WANG, G. SAGARDUI AND L. ETXEBERRIA. Test Case Prioritization of Configurable Cyber-Physical Systems with Weight-Based Search Algorithms. In Genetic and Evolutionary Computation Conference (GECCO)., 2016.
...
% TODO: I think that this also needed a subscript i on the variable x. Can this issue be further clarified?
...
computed for the complete vector $\vec{x}$ currently being optimized by the AVM, where $\vec{x}_i := x, 1 \leq i \leq len$. $F$ is the Fibonacci sequence starting from $F_0\,=\,0$. Each line is annotated to show the algorithm(s) to which it belongs.}
\vspace{-1em}
\end{figure}
...
As part of future work, we plan to extend the example set with case studies showing how the AVM is or can be applied to real SBSE problems, such as test data generation. These will be made available via the code repository.
