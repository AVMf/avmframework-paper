...
\pagerule
\vspace{-1em}
\caption{\label{fig:alg}IPS, LS, GS search algorithms for the AVM for a variable $x \in D$. $obj(x)$ denotes the problem-specific objective function, which is computed for the complete vector $\vec{x}$ currently being optimized by the AVM, where $\vec{x}_i := x, 0 \leq i \leq len$. Each line is annotated to show to which particular algorithm(s) it belongs.}
\vspace{-1.5em}
\end{figure}
