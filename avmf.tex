...
\caption{\label{fig:alg}IPS, LS, GS algorithms
%for the AVM
for a variable $x$. For ease of presentation
%, and without loss of generality,
we assume $x \in \mathbb{Z}$.
$obj(x)$
denotes objective function computation for the overall vector $\vec{x}$ being optimized, where $x_i$ is assigned to the value of $x$. $F$ is the Fibonacci sequence starting from $F_0=0$. Each line is annotated to show the algorithm(s) to which it belongs.}
\vspace{-1em}
\end{figure}
