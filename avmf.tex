...
%for the AVM
for a variable $x_i$. For ease of presentation
%, and without loss of generality,
we assume $x_i \in \mathbb{Z}$.
$obj(v)$
denotes objective function computation for the overall vector $\vec{x}$ with $v$ as the value of $x_i$. $F$ is the Fibonacci sequence starting from $F_0=0$. Each line is annotated to show the algorithm(s) to which it belongs.}
\vspace{-1em}
\end{figure}
