...
This paper introduced \name, an open-source implementation of the AVM and a framework supporting its use in SBSE projects. \name offers features of associated with cutting-edge local search techniques, such as a representation for a wide variety of data types and the implementations of new variable search algorithms. Since a well-documented version of \name is now freely available for download at \repourl, we judge that it will advance the use of the AVM in both industrial practice and in the SBSE research community. Moreover, \name's extensible object-oriented design will enable others to implement many different search-based solutions to problems in the field of software engineering. Possible future applications of the AVM include the following:

\inlineheading{Automatically Generating Readable Test Data} Generating readable test cases that humans can easily understand has been of recent interest to researchers in search-based testing (e.g., rewarding inputs that obtain a high score from a language model~\cite{Afshan2013}). More readable values were requested by participants of a recent study evaluating test generation tools~\cite{Fraser2013b}. Given that the AVM employs a local search, it could start with examples of human-generated inputs and adapt them to new coverage targets --- all without necessarily losing the qualities of the original data.
